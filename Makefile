...
BUILD_FOLDER = ./build
PROJECT_URL  = github.com/duyanghao/GWorkerPool
## command
GO           = go
GO_VENDOR    = go mod
MKDIR_P      = mkdir -p
...
.PHONY: src.build
src.build:
	cd cmd && GO111MODULE=on $(GO) build -mod=vendor -v -o ../$(BUILD_FOLDER)/GWorkerPool/GWorkerPool
...
dockerfiles.build:
	docker build --tag duyanghao/gworkerpool:$(SERVER_VERSION) -f ./docker/Dockerfile .
